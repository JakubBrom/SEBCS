...
# Name: SEBCS_SA                                                               #
# Modul: SEBCSlib.py                                                       #
# Autor: Dr. Jakub Brom, University of South Bohemia, Faculty of Agriculture   #
...
class GeoIO:
	...
	def readGeo(self, rast, band = 1):
		...
		:type rast: str
		:param band: Band number. Default is band = 1.
		...
		:rtype: float
		:returns: Pixel heigth (m) on Y axis.
		:rtype: float
	...
	def arrayToRast(self, arrays, names, prj, gtransf, EPSG, out_folder,
					driver_name = "GTiff", out_file_name = None,
					multiband = False):
...
class SolarRadBalance:
	...
	def slopeAspect(self, DMT, xSize, ySize):
		"""
	...
	def solarInTopo(self, Rs_in, slope, aspect, latitude, longitude, date_acq, time_acq):
		...
		sin_alpha = (math.sin(L_rad) * math.sin(ds_rad) + math.cos(L_rad)
					* math.cos(ds_rad) * math.cos(hs_rad))							# sin of solar heigth angle
		if sin_alpha < 0.0:
			sin_alpha = 0.0
	...
	def	downRL(self, ta, emis_a):
		...
		try:
			...
			raise ArithmeticError("Downward longwawe radiation flux has not been calculated.")
	...
	def	outRL(self, ts, emiss):
		...
		RL_out = emiss * 5.6703*10.0**(-8.0) * (Ts_K + 273.16)**4

		return RL_out


	def	albedoBrom(self, ndvi, msavi, a = 0.08611, b = 0.894716, c = 5.558657,
					d = -0.11829, e = -1.9818, f = -4.50339, g = -11.4625,
					h = 7.461454, i = 5.299396, j = 4.76657, k = -2.3127, l = -3.42739):
		"""
		...
		:param a: constant. Default a = 0.08611
		:type a: float
		:param b: constant. Default a = 0.894716
		:type b: float
		:param c: constant. Default a = 5.558657
		:type c: float
		:param d: constant. Default a = -0.11829
		:type d: float
		:param e: constant. Default a = -1.9818
		:type e: float
		:param f: constant. Default a = -4.50339
		:type f: float
		:param g: constant. Default a = -11.4625
		:type g: float
		:param h: constant. Default a = 7.461454
		:type h: float
		:param i: constant. Default a = 5.299396
		:type i: float
		:param j: constant. Default a = 4.76657
		:type j: float
		:param k: constant. Default a = -2.3127
		:type k: float
		:param l: constant. Default a = -3.42739
		:type l: float
		...
		albedo = (a + b * msavi + c*msavi**2 + d * ndvi + e * msavi**3
				+ f * msavi *ndvi + g * msavi**2 * ndvi
				+ h * msavi * ndvi**2 + i * msavi**2 * ndvi**2
				+ j * msavi**3 * ndvi + k * msavi**3 * ndvi**2
				+ l * msavi * ndvi**3)
...
class VegIndices:
	...
	def vi_SAVI(self, red, nir, L = 0.5):
		"""
...
class MeteoFeatures:
	...
	def airTemp(self, Z, Z_st, ta_x):
		"""
		Air temperature, recalculated on heigth Z (m) from data measured in heigth Z_st (m).
		ta_x - air temperature measured in heigth Z_st (°C)
		ta - air temperature measured in heigth Z (°C)
		"""
	...
	def vapourPress(self, E_sat, Rh):
		"""
		Function returns layer of water vapour pressure in air (kPa) at certaion level.
		"""
	...
	def	emissivity(self, Fc, red, ndvi):
		...
		emis = 0.004 * Fc + 0.986
		emis = np.where(ndvi < 0.2, 1 - red, emis)            # replacement of emissivity values for NDVI < 0.2 by values from red band 
		emis = np.where(ndvi > 0.5, 0.99, emis)              # replacement of emissivity values for NDVI > 0.5 by 0.99
		emis[emis > 1] = 0.99                                # replacement of values > 1 by 0.99
		emis[emis < 0.8] = 0.8								 # replacement values < 0.8 by 0.8
...
class WindStability(MeteoFeatures, HeatFluxes):
	...
	def	vegHeight(self, h_min, h_max, msavi):
		"""
		Heigth of vegetation cover (m) derived from MSAVI index
	...
	def	windSpeedZ(self, U, Z = 200.0, Z_st = 2.0, h_st = 0.12):
		...
		:type U: float
		:param Z: Blending heigth (mixing layer height) (m).
				  Default 200 m.
		:type Z: float
		:param Z_st: Heigth of wind speed measurement (m). Default 2 m.
		:type Z_st: float
		:param h_st: Heigth of vegetation cover under meteostation (m).
	...
	def psiM(self, L, X, Z = 200, a = 1.0, b = 0.667, c = 5.0, d = 0.35):
		...
		try:
			...
			raise ArithmeticError("Stability coefficient for momentum transfer for stable conditions has not been calculated")

		try:
			psi_m_unstab = 2.0 * np.log((1.0 + X)/2.0) + np.log((1.0 + X**2.0)/2.0) - 2.0 * np.arctan(X) + np.pi/2.0
		except ArithmeticError:
			raise ArithmeticError("Stability coefficient for momentum transfer for unstable conditions has not been calculated")
	...
	def psiH(self, L, X, Z = 200, a = 1.0, b = 0.667, c = 5.0, d = 0.35):
		...
		try:
			psi_h_stab = -((1 + 2 * a * dzeta/3)**1.5 + b * (dzeta - c/d) * np.exp((-d) * dzeta) + (b * c/d - 1))
		except ArithmeticError:
			raise ArithmeticError("Stability coefficient for heat transfer for stable conditions has not been calculated")

		try:
			psi_h_unstab =  2.0 * np.log((1.0 + X**2.0)/2.0)
		except ArithmeticError:
			raise ArithmeticError("Stability coefficient for heat transfer for unstable conditions has not been calculated")
	...
	def stabCoef(self, Uz, ta, ts, z0m, z0h, Z = 200, L = -10000.0, n_iter = 10, a  = 1.0, b = 0.667, c = 5.0, d = 0.35, kappa = 0.41, gravit = 9.81):
		"""
